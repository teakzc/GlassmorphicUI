...
function Utils.getEditableImage(source: string | Instance): (EditableImage?, boolean?)
	...
	local success, editableImage = pcall(function()
		...
		local ImageResize = AssetService:CreateEditableImage({ image.Size * Utils.IMAGE_DOWNSCALE_FACTOR  })
		ImageResize:DrawImageTransformed(
			Vector2.new(0, 0), -- Position
			image.Size * Utils.IMAGE_DOWNSCALE_FACTOR, -- Scale
			0, -- Rotation (degrees)
			image, -- Source image
			{
				CombineType = Enum.ImageCombineType.Overwrite, -- Optional, default is AlphaBlend
			}
		)
		image:Destroy()
		image = ImageResize
